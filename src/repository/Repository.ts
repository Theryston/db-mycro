...
export class Repository {
    ...
    for (let i in datas) {
      ...
      if (!this.columns || !this.columns.length) {
        ...
        for (let j in this.columns) {
          if (
            this.columns[j].autoIncrement &&
            this.columns[j].type != "number"
          ) {
            throw new Error(
              `the type of the '${this.columns[j].name}' value must be 'number' to use autoIncrement`
            );
          }
          ...
          if (this.columns[j].autoIncrement) {
            if (!this.datas[this.datas.length - 1]) {
              ...
              datas[i][this.columns[j].name] =
                this.datas[this.datas.length - 1][this.columns[j].name] + 1;
            }
          }
          ...
          if (
            typeof datas[i][this.columns[j].name] != this.columns[j].type ||
            (Array.isArray(datas[i][this.columns[j].name]) &&
              this.columns[j].type != "array")
          ) {
            throw new Error(
              `invalid data type to add in column '${this.columns[j].name}' in repository '${this.name}' of db-mycro`
            );
          }

          if (
            !datas[i][this.columns[j].name] &&
            !this.columns[j].allowNull &&
            !this.columns[j].default &&
            !this.columns[j].autoIncrement
          ) {
            throw new Error(
              `invalid data to add in column '${this.columns[j].name}' in repository '${this.name}' of db-mycro`
            );
          }

          if (this.columns[j].isPrimary) {
            const existsPrimary = this.datas.find(
              (d) => d[this.columns[j].name] == datas[i][this.columns[j].name]
            );
            if (existsPrimary) {
  ...
  public update(params: { where: string; data: object }) {
    ...
    for (let j in this.columns) {
      ...
      if (
        (params.data[this.columns[j].name] &&
          typeof params.data[this.columns[j].name] != this.columns[j].type) ||
        (Array.isArray(params.data[this.columns[j].name]) &&
          this.columns[j].type != "array")
      ) {
        throw new Error(
          `invalid data type to update in column '${this.columns[j].name}' in repository '${this.name}' of db-mycro`
        );
      }
      ...
      for (let i = 0; i <= datas.length; i++) {
        const index = this.datas.findIndex(
          (data, j) => eval(params.where) && j > indexes[indexes.length - 1]
        );
        indexes.push(index);
      }
      indexes.splice(0, 1);
      ...
      for (let i in indexes) {
        if (params.data[this.columns[j].name] != null) {
          this.datas[indexes[i]][this.columns[j].name] =
            params.data[this.columns[j].name];
        }
      }
    }
  ...
  public delete(where?: string) {
    try {
      const indexes = [];
      const datas = this.datas.filter((data, i) => {
        if (eval(where)) {
          indexes.push(i);
          return true;
        }
